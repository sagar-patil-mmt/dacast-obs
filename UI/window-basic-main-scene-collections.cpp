...
void EnumSceneCollections(std::function<bool (const char *, const char *)> &&cb)
{
	...
	int ret = GetConfigPath(path, sizeof(path),
			"dacast-obs-studio/basic/scenes/*.json");
	if (ret <= 0) {
		...
		std::string &file, const char *oldName = nullptr)
{
	...
	ret = GetConfigPath(path, sizeof(path), "dacast-obs-studio/basic/scenes/");
	if (ret <= 0) {
...
void OBSBasic::on_actionRenameSceneCollection_triggered()
{
	...
	char path[512];
	int ret = GetConfigPath(path, 512, "dacast-obs-studio/basic/scenes/");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get scene collection config path");
		return;
	}
